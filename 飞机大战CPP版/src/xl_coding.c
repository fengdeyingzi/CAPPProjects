...
int32 un_wstrlen(const char* str) {
    if (str == NULL) {
        return 0; // 空指针返回 0
    }
...
// 将utf16-be与utf16-le互转
char *un_swap(char *text){
    if (text == NULL) {
        return NULL; // 检查空指针
    }

    size_t len = un_wstrlen(text); // 获取字符串长度
    if (len == 0) {
        return NULL; // 空字符串
    }

    // 遍历每个 UTF-16 编码单元（2 字节）
...
char *unToGb(char *text)
{
    ...
    uint8 *output = malloc(input_len + 2);
    printf("input_len %d\n", input_len);
