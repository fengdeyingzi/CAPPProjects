...
#define SCRH (getScreenHeight())
    ...
    // WSTRLEN wstrlen;
    // 计算以 \0\0 结尾的 UTF-16 字符串的长度
int32 app_wstrlen(char* str) {
    if (str == NULL) {
        return 0; // 空指针返回 0
    }

    int32 len = 0;
    while (str[len] != 0 || str[len + 1] != 0) { // 检查两个字节是否都为 0
        len++;
    }
    return len;
}
    ...
    /*
 * �? GBK 字符串转换为 Unicode 字符串（大端�?
 * 输入:
 *   cp: 输入�? GBK 字符�?
 *   err: 若为�? NULL，则在转换出错时返回出错字符的索�?
 *   size: 输出�? Unicode 字符串长度（以字符为单位�?
 * 输出:
 *   err: 若为�? NULL，则在转换出错时返回出错字符的索�?
 *   size: 输出�? Unicode 字符串长�?
 * 返回:
 *   NULL: 转换出错
...
/*
 * �? Unicode 字符串（大端）转换为 GBK 编码
 * 输入:
 *   input: 输入�? Unicode 字符串（大端�?
 *   input_len: 输入�? Unicode 字符串长度（以字节为单位�?
 *   output: 输出�? GBK 字符串缓冲区（由调用者分配和管理�?
...
int32 mrc_unicodeToGb2312(uint8* input, int32 input_len, uint8** output, int32* output_len) {
    ...
    // 检查输入长度是否为偶数（UTF-16 每个字符�? 2 字节�?
    if (input_len % 2 != 0) {
        printf("mrc_unicodeToGb2312 len error\r\n");
        return MR_FAILED;
    }

    // 将大�? UTF-16 转换为小�? UTF-16
...
int log_printf(const char *format, ...) {
    char temp[5000];
    va_list args;
    va_start(args, format); // ��ʼ�� va_list

    
    int result = vsnprintf(temp, 5000, format, args);
