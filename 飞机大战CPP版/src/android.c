...
READFILEFROMASSETS readFileFromAssets_ptr;
void* readFileFromAssets(char *filename, int *len){
   void* data = readFileFromAssets_ptr(filename, len);
   void* redata = NULL;
   printf("readFile len = %d", *len);
   if(data != NULL && *len != 0){
    redata = malloc(*len + 1);
    memset(redata, 0, *len + 1);
    memcpy(redata, data, *len);
    printf("readFile %s", (char*)redata);
...
int android_add(char* name, void* ptr)
{
 ...
 {
 readFileFromAssets_ptr=(READFILEFROMASSETS)ptr;
 }
