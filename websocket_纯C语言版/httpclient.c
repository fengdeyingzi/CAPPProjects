...
//1字节
		...
		if(websocket->buffer->len <= 2)return NULL;
		FIN = (data[0] & 0x80)>>7;
		RSV1 = (data[0] & 0x40)>>6;
		RSV2 = (data[0] & 0x20)>>5;
		RSV3 = (data[0] & 0x10)>>4;
		...
		ptr = 2;
		if(payload_len == 0x7e){
			if(websocket->buffer->len <= ptr+2)return NULL;
			//接下来的2字节
			payload_len = readShort(data, 2);
			ptr+=2;
		}else if(payload_len == 0x7f){
			if(websocket->buffer->len <= ptr+8)return NULL;
			//接下来的8字节
		...
		if(mask == 1){ //存在掩码
			...
			if(websocket->buffer->len < total_len)return NULL;
