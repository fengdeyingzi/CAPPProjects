...
void* fnt_create(char* imgname, char* fntname) {
	...
	*(text + len) = 0;
	...
	fnt->font = malloc(line * sizeof(_FONT*));
	memset(fnt->font, 0, line * sizeof(_FONT*));
	fnt->size = line;
	//fnt->bitmap = readBitmapFromAssets(imgname);
		...
		for (int i = 0; i < fnt->size; i++) {
			if (fnt->font[i] != NULL) {
				if (id == 0x0a) {
					...
					//else
					{
						drawBitmapFlip(fnt->bitmap,
									   ix, iy, fnt->font[i]->width,
									   fnt->font[i]->height,
									   fnt->font[i]->x,
									   fnt->font[i]->y);
						ix += fnt->font[i]->width;
...
int fnt_getLineW(_FNT* fnt, char* text) {
	int w = 0, id = 0;
	char* untemp = ex_coding(text, strlen(text),
							 "UTF-8", "UTF-16LE");
