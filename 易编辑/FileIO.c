...
{
    return app_open(ConvertPathSeparator(filename), mode);
}

//创建文件夹
int32 mrc_mkDir_s(const char *name)
{
    return app_mkdir(ConvertPathSeparator(name));
}

//删除文件夹
int32 mrc_rmDir_s(const char *name)
{
    return app_rmdir(ConvertPathSeparator(name));
}

//重命名文件(夹)
int32 mrc_rename_s(const char* oldname, const char* newname)
{
    return app_rename(ConvertPathSeparator(oldname), ConvertPathSeparator1(newname));
}

//删除文件
int32 mrc_remove_s(const char* filename)
{
    return app_remove(ConvertPathSeparator(filename));
}

//取得文件类型信息
int32 mrc_fileState_s(const char* filename)
{
    return filestate(ConvertPathSeparator(filename));
}

//取得文件长度
int32 mrc_getLen_s(const char* filename)
{
    return app_getlen(ConvertPathSeparator(filename));
}

//读取文件的所有内容
void* mrc_readAll_s(const char* filename, uint32 *len)
{
    return readall(ConvertPathSeparator(filename), len);
}
